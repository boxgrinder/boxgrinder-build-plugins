...
module BoxGrinder
  describe EC2Plugin do
    before(:each) do
      ...
      @plugin = EC2Plugin.new.init(@config, @appliance_config, :previous_deliverables => OpenCascade.new(:disk => 'a/disk.raw'), :log => LogHelper.new(:level => :trace), :plugin_info => {:class => BoxGrinder::EC2Plugin, :type => :platform, :name => :ec2, :full_name => "Amazon Elastic Compute Cloud (Amazon EC2)"})
