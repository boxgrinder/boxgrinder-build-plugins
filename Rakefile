...
plugins = {
        ...
        "boxgrinder-build-rpm-based-os-plugin"    => { :version => '0.0.3', :dir => "os/rpm-based", :desc => 'RPM Based Operating System Plugin' },
        ...
        "boxgrinder-build-rhel-os-plugin"         => { :dir => "os/rhel", :desc => 'Red Hat Enterprise Linux Operating System Plugin', :deps => { 'boxgrinder-build-rpm-based-os-plugin' => '>=0.0.2' }},
        ...
        "boxgrinder-build-ec2-platform-plugin"    => { :version => '0.0.2', :dir => "platform/ec2", :desc => 'Elastic Compute Cloud (EC2) Platform Plugin' }
}
...
Echoe.new('#{name}') do |p|
  ...
  p.version     = '#{info[:version].nil? ? '0.0.1' : info[:version] }'
  p.summary     = '#{info[:desc]}'
